...
class Nilaidetilskirpsi extends CActiveRecord {
    ...
    public function rules() {
        ...
        return array(
            ...
            array('idNilaiSkripsi, IdPendaftaran,NIM, NilaiPenguji1,NominalVakasi,statusVakasi,bulan,tahun NIlaiPenguji2,NPraSidang,idjenisSidang', 'safe', 'on' => 'search'),
        );
    }
    ...
    , `prd_sidangmaster`.`IDJenisSidang`
    , `prd_mahasiswa`.`Nama`
    , `prd_pengujiskripsi`.`idPengujiSkripsi`
    , `prd_pengujiskripsi`.`idPendaftaran`
    , `prd_pengujiskripsi`.`idUser`
    , `prd_jenissidang`.`NamaSidang`
    , `prd_jenissidang`.`NominalVakasi`
    , `prd_nilai_penguji`.`nilaiSkripsi`
FROM
    ...
    left join prd_nilai_penguji on (prd_pengujiskripsi.idPengujiSkripsi=prd_nilai_penguji.idPengujiSkripsi)
...
FROM
    ...
    public function getTotalSkripsi($id) {
        // $id = implode("','", $id);
        ...
        INNER JOIN sttitpi_skkp.prd_pendaftaran ON (prd_pendaftaran.IdSidang = prd_sidangmaster.IdSidang)
        INNER JOIN sttitpi_skkp.prd_nilaidetilskirpsi ON (prd_nilaidetilskirpsi.IdPendaftaran = prd_pendaftaran.idPendaftaran)
        INNER JOIN sttitpi_skkp.prd_pengujiskripsi ON (prd_pengujiskripsi.idPendaftaran = prd_pendaftaran.idPendaftaran)
        INNER JOIN sttitpi_skkp.prd_mahasiswa ON (prd_pendaftaran.NIM = prd_mahasiswa.NIM)
        WHERE prd_sidangmaster.IDJenisSidang IN(1,2) AND prd_pengujiskripsi.idUser IN('$id')
        ORDER BY prd_sidangmaster.Tanggal DESC";
        $connection = Yii::app()->db->createCommand($sql)->queryScalar();
        return Yii::app()->format->number($connection);
    }
